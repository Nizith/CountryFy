...
export default function CountryCard({ commonName, officialName, flagUrl, countryData }) {
    ...
    return (
        <motion.div
            className='bg-white p-4 border-2 border-gray-300 rounded-lg hover:bg-indigo-50 hover:border-indigo-500 cursor-pointer shadow-md'
            onClick={handleViewMore}
            initial={{ y: 0, scale: 1 }} // Initial position and scale
            animate={{ y: 0, scale: 1 }} // Default position and scale when not hovered
            whileHover={{
                scale: [1, 1.10, 1, 1.05], // Keyframes for scaling
                transition: {
        ...
        >
            <h2 className='text-xl font-bold text-indigo-700'>{commonName}</h2>
            <p className='text-gray-600 text-sm'>{officialName}</p>
